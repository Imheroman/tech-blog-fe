export interface Post {
  ...
  content: string;
  category: string;
  /** 발행 시각. RFC 3339(ISO 8601) UTC 문자열. 정렬·표시 기준. */
  publishedAt: string;
  /** 수정 시각. 선택적. RFC 3339(ISO 8601) UTC 문자열. */
  updatedAt?: string;
  /** 읽기 시간(분). 표시 문구("N min read")는 컴포넌트에서 생성. */
   ...
   */
  views: number;
  /** 게시 상태. 공개 API는 항상 "published"만 노출. */
...
function formatDate(iso: string) {
  ...
  return date.toLocaleDateString("ko-KR", {
    ...
    month: "long",
    day: "numeric",
    // 서버/클라이언트 hydration 불일치 방지를 위해 타임존 고정.
    timeZone: "Asia/Seoul",
  });
}
...
export { formatDate, formatReadTime };
